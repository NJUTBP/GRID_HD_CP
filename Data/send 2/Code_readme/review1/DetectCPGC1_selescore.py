...
for ratio in ratios:
    for s in session_list:
        ...
        num_grid_sele=int(len(grid_idx)*ratio)
        random_selection = random.sample(grid_idx, num_grid_sele)
        ordered_selection = sorted(random_selection, key=grid_idx.index)
        cellidx['grid'] = ordered_selection
        ...
        border_idx = np.arange(num_border)
        grid_idx = np.arange(num_border, num_border + num_grid_sele)
        hd_idx = np.arange(num_border + num_grid_sele, num_border + num_grid_sele + num_hd)
        ov_idx = np.arange(num_border + num_grid_sele + num_hd, num_border + num_grid_sele + num_hd + num_ov)
        other_idx = np.arange(num_border + num_grid_sele + num_hd + num_ov, num_border + num_grid_sele + num_hd + num_ov + num_other)
        cellidx = {}
        cellidx['border'] = border_idx.tolist()
        cellidx['grid'] = grid_idx.tolist()
        cellidx['hd'] = hd_idx.tolist()
        ...
        plt.tight_layout()
        plt.savefig(pathout+s+'_'+str(ratio)+'.png', dpi=300, bbox_inches='tight')
        ...
        output['tpr']=tpr     
        output['auc']=auc     
        with open(pathout+s+'_'+str(ratio)+'.pickle', 'wb') as file: #w -> write; b -> binary
