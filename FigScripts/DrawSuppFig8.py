...
def main():
    ...
    for no,s in enumerate(session_list):
        ...
        sc=ax4.scatter(df['GS'][::-1],df['CS'][::-1],c=df['MIC'][::-1],cmap='jet',s=10)
        ...
        cax.set_xlabel('Spatial information content (bit/D.F.)', labelpad=1)
        
        color_dict = {'NGC': 'tab:olive', 'GC': 'tab:cyan'}
        sns.stripplot(data=df, x="GCG", y="MIC", order=['GC', 'NGC'], palette=color_dict, ax=ax5a, alpha=0.5)
        sns.boxplot(data=df, x="GCG", y="MIC", order=['GC', 'NGC'], palette=color_dict, ax=ax5a, boxprops={'facecolor':'None'}, showfliers=False)
        pairs = [("GC", "NGC")]
        annotator = Annotator(ax5a, pairs, data=df, x="GCG", y="MIC", order=['GC', 'NGC'],)
        ...
        sns.stripplot(data=df, x="CPG", y="MIC", order=['Core', 'Peri'], palette=color_dict, ax=ax5b, alpha=0.5)
        sns.boxplot(data=df, x="CPG", y="MIC", order=['Core', 'Peri'], palette=color_dict, ax=ax5b, boxprops={'facecolor':'None'}, showfliers=False)
        pairs = [("Core", "Peri")]
        annotator = Annotator(ax5b, pairs, data=df, x="CPG", y="MIC", order=['Core', 'Peri'],)
        annotator.configure(test='t-test_ind', text_format='star', line_height=0.03, line_width=1)
        annotator.apply_and_annotate()
        ...
        ax5a.set_xlabel('')
        ax5a.set_ylabel('Spatial information content (bit/D.F.)')
    ...
    plt.savefig('../Figures/SuppFigure8.png',format='PNG',dpi=300)
    plt.show()
    plt.close()
